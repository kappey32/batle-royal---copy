...
function jugador_2 () {
    ...
    statusbar21 = statusbars.create(20, 4, StatusBarKind.Health)
    statusbar21.value = 100
    statusbar21.attachToSprite(player_2)
    statusbar21.setColor(9, 2)
    statusbar21.setBarBorder(1, 15)
    statusbar21.setLabel("HP", 15)
    staturbar22 = statusbars.create(20, 4, StatusBarKind.Magic)
    staturbar22.value = 100
    staturbar22.attachToSprite(mySprite)
    staturbar22.setColor(5, 2)
    staturbar22.setBarBorder(1, 15)
    staturbar22.setLabel("mg", 15)
    staturbar22.positionDirection(CollisionDirection.Bottom)
    tiles.placeOnTile(player_2, tiles.getTileLocation(30, 30))
...
let staturbar22: StatusBarSprite = null
let statusbar21: StatusBarSprite = null
...
let players = 4
// Player variables
let player1 = sprites.create(assets.image`player1`)
let player2 = sprites.create(assets.image`player2`)

// Camera variables
let player1Camera = scenecamera2(`player1`)
let player2Camera = scenecamera,(`player2`)

// Function to follow player with camera
function followPlayer(player, camera) {
    camera.x = player.x - scene.screenWidth() / 2
    camera.y = player.y - scene.screenHeight() / 2
}

// Main game loop
game.onUpdate(function () {
    // Update camera position for each player
    followPlayer(player1, player1Camera)
    followPlayer(player2, player2Camera)
})
